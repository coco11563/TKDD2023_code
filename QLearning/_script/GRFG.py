...
from QLearning._model.DQN import DQN1, DQN2, DDQN1, DDQN2, DuelingDQN1, DuelingDQN2, DuelingDDQN1, DuelingDDQN2
from QLearning._utils.tools import *
from task_mapping import task_dict, task_type, base_path, task_measure, state_rep, support_rl_method
import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--file_name', type=str, default='openml_589', help=
    'data name')
...
parser.add_argument('--rl_method', type=str, default='dqn', help=
    'used reinforcement methods')
parser.add_argument('--state_method', type=str, default='ae', help=
    'reinforcement state representation method')
parser.add_argument('--default_cuda', type=int, default=-1, help=
    'the using cuda')
...
if not os.path.exists('./tmp'):
    os.mkdir('./tmp/')
if not os.path.exists('./tmp/' + param.file_name):
    os.mkdir('./tmp/' + param.file_name)
if not os.path.exists('./log'):
    os.mkdir('./log/')
if not os.path.exists('./log/' + param.file_name):
    os.mkdir('./log/' + param.file_name)
start_time = str(time.asctime())
log_file = ('./log/' + param.file_name + '/' + start_time + '_' +
    STATE_METHOD + '.log')
D_OPT_PATH = './tmp/generated_best_features_' + param.file_name + '-' + str(
    param.index) + '-' + start_time + '.csv'
OVER_ALL_D_PATH = ('./tmp/generated_overall_features_' + param.file_name +
    '-' + str(param.index) + '-' + start_time + '.csv')
from QLearning._utils.logger import *
logging.basicConfig(filename=log_file, level=logging_level[param.log_level],
...
if param.rl_method == 'dqn':
    ...
    dqn_operation = DQN2(N_STATES=STATE_DIM, N_ACTIONS=N_ACTIONS,
        MEMORY_CAPACITY=MEMORY_CAPACITY)
    dqn_cluster2 = DQN1(STATE_DIM=STATE_DIM + OP_DIM, ACTION_DIM=ACTION_DIM,
        MEMORY_CAPACITY=MEMORY_CAPACITY)
    ...
    dqn_cluster1 = DuelingDQN1(STATE_DIM=STATE_DIM, ACTION_DIM=ACTION_DIM,
        MEMORY_CAPACITY=MEMORY_CAPACITY)
    ...
    dqn_cluster1 = DuelingDDQN1(STATE_DIM=STATE_DIM, ACTION_DIM=ACTION_DIM,
        MEMORY_CAPACITY=MEMORY_CAPACITY)
    dqn_operation = DuelingDDQN2(N_STATES=STATE_DIM, N_ACTIONS=N_ACTIONS,
        MEMORY_CAPACITY=MEMORY_CAPACITY)
    dqn_cluster2 = DuelingDDQN1(STATE_DIM=STATE_DIM + OP_DIM, ACTION_DIM=
        ACTION_DIM, MEMORY_CAPACITY=MEMORY_CAPACITY)
info('perform base test with random forest classifier')
...
info(
    ...
    .format(EPS_START, EPS_END, EPS_DECAY, CLUSTER_NUM, duplicate_count, a,
    b, c))
...
while episode < EPISODES:
    ...
    while step < STEPS:
        ...
        action_emb_c1, state_emb_c1, f_cluster1, f_names1 = (
            select_meta_cluster1(clusters, Dg.values[:, :-1], feature_names,
            eps_threshold, dqn_cluster1, STATE_METHOD, DEVICE))
        ...
        r_c1, r_op, r_c2 = a * reward / 3, b * reward / 3, c * reward / 3
        if new_per > best_per:
            bset_step = step
        ...
        action_emb_c1_, state_emb_c1_, f_cluster_, clusters_ = (
            ...
            dqn_cluster1, cluster_num=CLUSTER_NUM, method=STATE_METHOD, gpu
            =DEVICE))
        state_emb_op_, op_ = generate_next_state_of_meta_operation(f_cluster_,
            operation_set, dqn_operation, method=STATE_METHOD, gpu=DEVICE)
        if op in O2:
            ...
            dqn_cluster2.store_transition(state_emb_c2, action_emb_c2, r_c2,
                state_emb_c2_, action_emb_c2_)
        ...
        info('Current spend time for step-{} is: {:.1f}s'.format(step, time
...
if task_name == 'reg':
    ...
    info('MAE on original is: {:.3f}, MAE on generated is: {:.3f}'.format(
        mae0, mae1))
    info('RMSE on original is: {:.3f}, RMSE on generated is: {:.3f}'.format
        (rmse0, rmse1))
...
if task_name == 'cls':
    ...
    acc1, precision1, recall1, f1_1 = test_task_new(D_OPT, task=task_name,
        state_num=0)
    info('Acc on original is: {:.3f}, Acc on generated is: {:.3f}'.format(
        acc0, acc1))
    info('Pre on original is: {:.3f}, Pre on generated is: {:.3f}'.format(
        precision0, precision1))
    info('Rec on original is: {:.3f}, Rec on generated is: {:.3f}'.format(
        recall0, recall1))
    info('F-1 on original is: {:.3f}, F-1 on generated is: {:.3f}'.format(
        f1_0, f1_1))
if task_name == 'det':
    ...
    info(
        ...
        .format(map0, map1))
    info('F1 Score on original is: {:.3f}, F1 Score on generated is: {:.3f}'
        .format(f1_0, f1_1))
    info(
        'ROC AUC Score on original is: {:.3f}, ROC AUC Score on generated is: {:.3f}'
        .format(ras0, ras1))
info('Total using time: {:.1f}'.format(time.time() - training_start_time))
info('Total using time: {:.1f}'.format(time.time() - training_start_time))
D_OPT.to_csv(D_OPT_PATH)
overall_D = overall_feature_selection(best_features, task_name)
overall_D.to_csv(OVER_ALL_D_PATH)
